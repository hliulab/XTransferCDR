...
class XTransferCDR2(nn.Module):
    def __init__(self, config, dtype=torch.float32):
        ...
        if config["dataset"] == "gears_double_gene":
            self.lambda_reco_3 = config["lambda_reco_3"]
    ...
    def add_noise(self, inputs: torch.Tensor, noise_factor=0.1):
        if (
            self.training
        ):  # model.train() 时 self.training 为 True, model.eval() 时 self.training 为 False
    ...
    def forward(
        ...
        if self.mode == "cell":
            ...
            treats_5 = self.decoder(base_left_noise, pert_left_noise+pert_right_noise)
        ...
        loss_5 = self.loss_autoencoder(treats_double, treats_5)

        loss = loss_1 * self.lambda_reco_1 + loss_4 * self.lambda_reco_2 + loss_5 * self.lambda_reco_3
        ...
        return loss, loss_1, loss_2, loss_3, loss_4, loss_5, loss_base_left, loss_base_right, dis, orthogonal_loss, embedding
    ...
    def double_gene_predict(
        self,
        treats_left,
        treats_right
    ):
        pert_left = self.encoderG_P(treats_left)
        base_left = self.encoderG_S(treats_left)

        pert_right = self.encoderG_P(treats_right)

        treats_cell_b_with_pert_1_cross = self.decoder(base_left, pert_left+pert_right)
